...
export default function ContactFormClient({ lang }: ContactFormClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Construct mailto URL (EN/RO) - use template literals everywhere for lint compliance
      const subject = lang === "ro"
        ? `Solicitare contact - ${data.phone ? encodeURIComponent(`${data.phone}`) : ""}${data.subject ? ` - ${encodeURIComponent(`${data.subject}`)}` : ""}`
        : `Contact request - ${data.phone ? encodeURIComponent(`${data.phone}`) : ""}${data.subject ? ` - ${encodeURIComponent(`${data.subject}`)}` : ""}`;
      const body = lang === "ro"
        ? `Nume: ${encodeURIComponent(`${data.name ?? ""}`)}%0ATelefon: ${encodeURIComponent(`${data.phone ?? ""}`)}%0AMesaj: ${encodeURIComponent(`${data.message ?? ""}`)}`
        : `Name: ${encodeURIComponent(`${data.name ?? ""}`)}%0APhone: ${encodeURIComponent(`${data.phone ?? ""}`)}%0AMessage: ${encodeURIComponent(`${data.message ?? ""}`)}`;
      const mailto = `mailto:contact@centralatermicaservice.ro?subject=${subject}&body=${body}`;
